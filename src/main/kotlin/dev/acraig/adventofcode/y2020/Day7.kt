...
data class BagDefinition(val name: String, val bagTypes: Map<String, Int>) {
    fun canContainShinyGold(allDefinitions:Map<String, BagDefinition>):Boolean {
        return bagTypes.containsKey("shiny gold") || bagTypes.keys.any { allDefinitions[it]!!.canContainShinyGold(allDefinitions) }
    }
...
fun day7Part1(path:Path):Int {
    ...
    return bagDefinitions.values.map() {
        bag -> bag.canContainShinyGold(bagDefinitions)
    }.count { it }
}
